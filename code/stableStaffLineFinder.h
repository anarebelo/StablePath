...
private:
	...
	inline weight_t weightFunction (int row1, int col1, int row2, int col2, stableStaffLineFinder::NEIGHBOUR neigh);
	...
	void postProcessing (std::vector <std::vector<Point2D> > &validStaves, int staffDistance, myIplImage* imgErodedCopy,const char *imgpath);
...
public:
	static void findGraySumStaffHeightAndDistance(myIplImage* grayimg, int& staffHeight, int& staffDistance);
	...
	stableStaffLineFinder (const char * imgpath);
	~stableStaffLineFinder ();
	void stableStaffDetection (std::vector <std::vector<Point2D> > &validStaves, const char * imgpath);
};
